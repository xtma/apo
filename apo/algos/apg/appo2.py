...
from apo.algos.apg.base import AveragePolicyGradientAlgo
...
LossInputs = namedarraytuple(
    "LossInputs", ["agent_inputs", "action", "return_", "advantage", "valid", "old_dist_info", "reward", "bv"])
OptInfo = namedtuple("OptInfo", ["loss", "eta_loss", "gradNorm", "entropy", "perplexity", "eta", "valueBias"])
...
class APPO(AveragePolicyGradientAlgo):
    ...
    def initialize(self, *args, **kwargs):
        ...
        self._eta = torch.zeros(1, requires_grad=True)
        self.eta_optimizer = self.OptimCls((self._eta,), lr=self.learning_rate, **self.optim_kwargs)
    ...
    def optimize_agent(self, itr, samples):
        ...
        for _ in range(self.epochs):
            for idxs in iterate_mb_idxs(batch_size, mb_size, shuffle=True):
                ...
                loss, eta_loss, entropy, perplexity = self.loss(*loss_inputs[T_idxs, B_idxs], rnn_state)
                loss.backward()
    ...
    def loss(
        ...
        eta_loss = valid_mean((reward - self._eta + self.discount * bv - value.detach())**2, valid)

        perplexity = dist.mean_perplexity(dist_info, valid)
        return loss, eta_loss, entropy, perplexity
