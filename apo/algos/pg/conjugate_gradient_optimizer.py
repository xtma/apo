...
This file is adapted from https://github.com/rlworkgroup/garage/blob/master/src/garage/torch/optimizers/conjugate_gradient_optimizer.py.
"""
...
def unflatten_tensors(flattened, tensor_shapes):
    ...
    return [tensor.reshape(shape) for tensor, shape in zip(torch.split(flattened, tensor_sizes), tensor_shapes)]
...
def _build_hessian_vector_product(func, params, reg_coeff=1e-5):
    ...
    def _eval(vector):
        ...
        grad_vector_product = torch.stack([(g * x).sum() for g, x in zip(f_grads, unflatten_vector)]).sum()
...
class ConjugateGradientOptimizer(Optimizer):
    ...
    def step(self, f_loss, f_constraint):  # pylint: disable=arguments-differ
        ...
        step_size = torch.sqrt(2.0 * self._max_constraint_value *
                               (1. / (torch.dot(step_dir, f_Ax(step_dir)) + 1e-8))).item()
    ...
    def _backtracking_line_search(self, params, descent_step, f_loss, f_constraint):
        ...
        if ((torch.isnan(loss) or torch.isnan(constraint_val) or loss >= loss_before or
             constraint_val >= self._max_constraint_value) and not self._accept_violation):
            ...
            if constraint_val >= self._max_constraint_value:
                logger.log('Violated because constraint is violated, '
                           f'{constraint_val} >= {self._max_constraint_value}')
