...
def discount_return(reward, done, bootstrap_value, eta, return_dest=None):
    ...
    return_[-1] = reward[-1] - eta + bootstrap_value * nd[-1]
    for t in reversed(range(len(reward) - 1)):
        return_[t] = reward[t] - eta + return_[t + 1] * nd[t]
    return return_
...
def generalized_advantage_estimation(reward,
    ...
    nd = 1 - done
    nd = nd.type(reward.dtype) if isinstance(nd, torch.Tensor) else nd
    advantage[-1] = reward[-1] - eta + bootstrap_value * nd[-1] - value[-1]
    for t in reversed(range(len(reward) - 1)):
        delta = reward[t] - eta + value[t + 1] * nd[t] - value[t]
        advantage[t] = delta + gae_lambda * nd[t] * advantage[t + 1]
